            ...
            ->schema([
                ...
                DatePicker::make('fecha_ingreso')
                    ...
                    ->afterStateHydrated(function ($component, $state) {
                        // Forzar carga del valor inicial al editar
                    ...
                    ->rule(function (callable $get, ?\App\Models\Ingreso $record) {
                        ...
                        return function (string $attribute, $value, \Closure $fail) use ($fechaIngreso, $fechaBaja, $operarioId, $record) {
                            $query = \App\Models\Ingreso::where('operario_id', $operarioId)
                                ->where(function ($q) use ($fechaIngreso, $fechaBaja) {
                                    $q->where(function ($q2) use ($fechaIngreso, $fechaBaja) {
                                        $q2->whereDate('fecha_ingreso', '<=', $fechaBaja ?? today())
                                           ->where(function ($q3) use ($fechaIngreso) {
                                               $q3->whereNull('fecha_baja')->orWhereDate('fecha_baja', '>=', $fechaIngreso);
                                           });
                                    });
                                });
